...
// function to handle the submit of the form and intercept the default event
  submitHandler = function (event) {
    ...
    if(Stripe){
      ...
      show_error("Failed to load credit card processing funcitonality. Please reload the page.")
    }
    return false;
  };
...
// function to handle even of plan drop down changing
...
// set up plan change event listener for #tenant_plan_id in the forms for class cc_form
...
// function to handle the token received from Stripe and remove credit card fields
  stripeResponseHandler = function (status, response) {
    ...
    if (response.error) {
      ...
      $("[data-strip=exp-year]").remove();
      $("[data-strip=exp-month]").remove();
      $("[data-strip=label]").remove();
      $form.get(0).submit();
    }
    return false;
  };
